...
def patternrecognition(input):
    ...
    if blobs == "":  # empty pattern
        ...
        counts = [get_count(pattern, blob) for blob in blobs.split("|")]
        return "{partial}|{total}".format(
            partial="|".join([str(c) for c in counts]), total=sum(counts)
        )
...
if __name__ == "__main__":
    ...
    answers = ["3|2|1|2|8", "4|4|2|0|10", "4|2|3|2|11", "0|0|0|0|0"]
