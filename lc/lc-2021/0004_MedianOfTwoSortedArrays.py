...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        if (n1 + n2) % 2 == 1:
            ...
            if m1 == 0:
                return (nums2[m2 - 1] + min(nums1[m1], nums2[m2])) / 2 if m2 < n2 else (nums2[m2 - 1] + nums1[m1]) / 2
            elif m1 == n1:
                return (max(nums1[m1 - 1], nums2[m2 - 1]) + nums2[m2]) / 2 if m2 >= 1 else (nums1[m1 - 1] + nums2[m2]) / 2
            elif m2 == 0:
                return (nums1[m1 - 1] + min(nums1[m1], nums2[m2])) / 2 if m1 < n1 else (nums1[m1 - 1] + nums2[m2]) / 2
            elif m2 == n2:
                return (max(nums1[m1 - 1], nums2[m2 - 1]) + nums1[m1]) / 2 if m1 >= 1 else (nums2[m2 - 1] + nums1[m1]) / 2
            else:
